import 'package:flutter/material.dart';

/// Brand and surface colors used across the app.
///
/// All color values are material-friendly and accessible.
class AppColors {
  AppColors._();

  // Primary Colors
  static const Color primary = Color(0xFF2563EB);
  static const Color primaryLight = Color(0xFF60A5FA);
  static const Color primaryDark = Color(0xFF1D4ED8);
  static const Color primarySurface = Color(0xFFEFF6FF);

  // Secondary Colors
  static const Color secondary = Color(0xFFF59E0B);
  static const Color secondaryLight = Color(0xFFFCD34D);
  static const Color secondaryDark = Color(0xFFD97706);

  // Neutral Colors
  static const Color white = Color(0xFFFFFFFF);
  static const Color black = Color(0xFF000000);
  static const Color background = Color(0xFFF8FAFC);
  static const Color surface = Color(0xFFFFFFFF);
  static const Color surfaceVariant = Color(0xFFF1F5F9);

  // Text Colors
  static const Color textPrimary = Color(0xFF1E293B);
  static const Color textSecondary = Color(0xFF64748B);
  static const Color textHint = Color(0xFF94A3B8);
  static const Color textOnPrimary = Color(0xFFFFFFFF);
  static const Color textOnDark = Color(0xFFFFFFFF);

  // Status Colors
  static const Color success = Color(0xFF10B981);
  static const Color successLight = Color(0xFFD1FAE5);
  static const Color error = Color(0xFFEF4444);
  static const Color errorLight = Color(0xFFFEE2E2);
  static const Color warning = Color(0xFFF59E0B);
  static const Color warningLight = Color(0xFFFEF3C7);
  static const Color info = Color(0xFF3B82F6);
  static const Color infoLight = Color(0xFFDBEAFE);

  // Request Status Colors
  static const Color statusPending = Color(0xFFF59E0B);
  static const Color statusAccepted = Color(0xFF3B82F6);
  static const Color statusInProgress = Color(0xFF8B5CF6);
  static const Color statusCompleted = Color(0xFF10B981);
  static const Color statusReviewed = Color(0xFF059669);
  static const Color statusRejected = Color(0xFFEF4444);

  // Rating Colors
  static const Color starFilled = Color(0xFFFBBF24);
  static const Color starEmpty = Color(0xFFE2E8F0);

  // Border
  static const Color border = Color(0xFFE2E8F0);
  static const Color borderLight = Color(0xFFF1F5F9);
  static const Color divider = Color(0xFFE2E8F0);

  // Shadow
  static const Color shadow = Color(0x1A000000);

  // Forest Green (from brand palette)
  static const Color forestGreen = Color(0xFF7193A3);

  // Category Colors
  static const Color categoryPlumbing = Color(0xFF3B82F6);
  static const Color categoryElectrical = Color(0xFFF59E0B);
  static const Color categoryCarpentry = Color(0xFF8B5CF6);
  static const Color categoryPainting = Color(0xFFEC4899);
  static const Color categoryMasonry = Color(0xFF6B7280);
  static const Color categoryCleaning = Color(0xFF10B981);
  static const Color categoryAppliance = Color(0xFFEF4444);
  static const Color categoryWelding = Color(0xFFF97316);
  static const Color categoryRepair = Color(0xFF06B6D4);
  static const Color categoryOther = Color(0xFF8B5CF6);
}
